...
class Banner(BigIntPrimaryKeyMixin, TimestampMixin, Base):
    ...
    body: Mapped[str | None] = mapped_column(Text)
    show_title: Mapped[bool] = mapped_column(default=False, nullable=False)
    show_subtitle: Mapped[bool] = mapped_column(default=False, nullable=False)
    show_body: Mapped[bool] = mapped_column(default=False, nullable=False)
    image_id: Mapped[int | None] = mapped_column(ForeignKey("media_assets.id"))
...
class ContentBlockItem(BigIntPrimaryKeyMixin, TimestampMixin, SortOrderMixin, Base):
    ...
    subtitle_zh: Mapped[str | None] = mapped_column(String(500), nullable=True)
    content: Mapped[str | None] = mapped_column(Text)
