...
POST_CATEGORIES_URL = "/api/v1/admin/post_categories"
...
def test_unrelated_entity_duplicate_conflict_still_returns_409(client, admin_headers: dict[str, str]) -> None:
    first_response = client.post(
        POST_CATEGORIES_URL,
        headers=admin_headers,
        json={"name": "Corporate News", "slug": "corporate-news", "status": "active"},
    )
    ...
    duplicate_response = client.post(
        POST_CATEGORIES_URL,
        headers=admin_headers,
        json={"name": "Corporate News Copy", "slug": "corporate-news", "status": "active"},
    )
